...
<body class="fix-header card-no-border fix-sidebar">
            ...
            <div class="container-fluid">
                ...
                <div class="row page-titles">
                    <div class="col-md-12 align-self-center">
                        <h3 class="text-themecolor">Results / Outstanding Persons</h3>
                    </div>
                </div>
                <!-- ============================================================== -->
                ...
                $query = "SELECT event_list.events, p_1.s_name, p_2.s_name, p_3.s_name, p_4.s_name, op.op_name_1, op.op_name_2, op.op_name_3, op.op_name_4, op.op_id, op.event_id
                ...
                if($stmt = $conn->prepare($query))
                {
                    for($i=0; $i<5; $i++)
                    {
                        ...
                        if($stmt->execute())
                        {
                            ...
                            if($stmt->num_rows())
                            {
                                $stmt->bind_result($event, $p1, $p2, $p3, $p4, $d1, $d2, $d3, $d4, $id, $event_id);
                                ?>
                            ...
                            {
                                ?>
                                <h2>No Outstanding Persons Found!</h2>
                                <?php
                            }
                        }
                        else
                        ...
                        ?>
                                    <div class="right-text">
                                        <a href="print/print_results_outstanding.php?age=<?php echo $age ?>&category=<?php echo $category ?>"><i class="fa fa-file-excel-o fa-3x pointer pointer"></i></a>
                                    </div>
                                    </div>
                                </div>
                            </div>
    ...
    <script type="text/javascript">
        ...
        function edit_func(id, category, age, event_id)
        {
            ...
            swal({
                title : "Are you sure?",
                text : "Edit the outstanding persons details",
                type : "info",
                ...
                if(isConfirm)
                {
                    $.redirect('add_winners.php', {id : id, age: age, category: category, edit : 'op', event_id : event_id})
                }
            });
        }
        ...
        function delete_func(id, location_id)
        {
            ...
            swal({
                    title: "Are you sure?",
                    text: "Delete the outstanding persons of the event",
                    showCancelButton: true,
                    ...
                    if (isConfirm) {
                        $.ajax({
                                type: "POST",
                                url: "submit/delete_outstanding_persons.php",
                                data: {
                                    id: id
                                }
                            })
